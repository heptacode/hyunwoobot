  ...
  try {
    sendEmbed(
      { member: member },
      {
        ...
        author: { name: states.get(member.guild.id).locale.log.guildMemberAdd, iconURL: props.icon.in },
    ...
    for (const autoRole of states.get(member.guild.id).autoRoles) {
      ...
      await sendEmbed(
        ...
        {
          color: props.color.cyan,
          author: { name: states.get(member.guild.id).locale.autoRole.roleAppended, iconURL: props.icon.role_append },
