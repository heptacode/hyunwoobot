  ...
  async execute(state: State, interaction: Interaction | any) {
    try {
      ...
      if (method === 'view') {
        ...
        const idx = state.voiceRoles.findIndex(
          (voiceRole: VoiceRole) => voiceRole.voiceChannel === voiceChannel
        );
        if (idx === -1)
        ...
        let description = payload.length
          ? `✅ **${state.locale.voiceRole.updated.replace('{cnt}', String(payload.length))}**`
          : `🙅 **${state.locale.voiceRole.noChanges}**\n`;
        payload.forEach(
      ...
      if (state.voiceRoles && state.voiceRoles.length >= 1)
        state.voiceRoles.forEach((voiceRole: VoiceRole) =>
          fields.push({
            ...
            value: `<@&${voiceRole.role}>${
              voiceRole.textChannel ? `(<#${voiceRole.textChannel}>)` : ''
            }`,
          })
        );
      ...
      return [
        {
          ...
          title: `**⚙️ ${state.locale.voiceRole.voiceRole}**`,
          fields:
            fields.length >= 1 ? fields : [{ name: '\u200B', value: state.locale.voiceRole.empty }],
        },
      ];
    } catch (err) {
