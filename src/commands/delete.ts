...
module.exports = {
  ...
  async execute(locale, dbRef, docRef, message, args) {
    try {
      if (!(message.member.hasPermission("ADMINISTRATOR") || message.member.hasPermission("MANAGE_MESSAGES"))) return message.channel.send(`${locale.insufficientPerms_delete}`);
      ...
      if (result) return message.channel.send(new Discord.MessageEmbed().setColor("#7788D4").setAuthor(`🗑 ${amount}${locale.delete}`).setFooter(message.author.tag).setTimestamp());
